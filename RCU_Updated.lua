...
for attempt = 1, maxRetries do
    local success, errorMsg = pcall(function()
        ...
        BuildUI = function()
            ...
            -- Carrega a UI do Christmas Event
            pcall(BuildChristmasUI)

            
            
-- ==================== CHRISTMAS EVENT UI ====================
local function BuildChristmasUI()
    local AutoHatch, AutoDamage
    ...
    -- Variable to track if we are in the special snowman mode
    local isBuildASnowmanMode = false
    ...
    selectedEgg1 = eggsTable[1] and eggsTable[1].name
    local defaultEgg = eggOptions[1]

    -- Add the special option
    table.insert(eggOptions, "Build a Snowman Egg")

    EggDropdown1 = EggHatchingSection:Dropdown("SelectEgg", {
        Title = "Select Egg",
        Values = eggOptions,
        Default = defaultEgg,
        Multi = false,
        Searchable = true,
        Callback = function(Value)
            if Value == "Build a Snowman Egg" then
                isBuildASnowmanMode = true
                selectedEgg1 = "Christmas" -- Target egg for snowman event
            else
                isBuildASnowmanMode = false
                selectedEgg1 = displayToEggName[Value] or (Value:match("^(.-) %-") or Value)
            end
        end
    })

    local hatchAmounts = {["1x"]=1, ["3x"]=3, ["Max"]=99}
    selectedAmount1 = hatchAmounts["Max"]
    HatchAmount = EggHatchingSection:Dropdown("SelectHatchAmount", {
        ...
        Callback = function(Value) selectedAmount1 = hatchAmounts[Value] or 1 end
    })
    ...
    local AutoHatch = EggHatchingSection:Toggle("AutoHatch", {
        ...
        Default = false,
        Callback = function(Value)
            _G.AutoHatchEnabled1 = Value
            if Value then
                task.spawn(function()
                    while _G.AutoHatchEnabled1 do
                        if not HatchingController._isHatching then
                            if isBuildASnowmanMode then
                                EggService.openEgg._re:FireServer("Christmas", selectedAmount1 or 1, {isBuildASnowman = true})
                            else
                                local args = {selectedEgg1, selectedAmount1 or 1}
    ...
    local ChristmasSection = Tabs.Christmas:AddSection("Build A Snowman")

    local autoBuildThread, originalToggleStates = nil, nil

    local isEventStartingSoon = false
    local inMinigameTimer = "N/A"

    local snowmanTimerParagraph = ChristmasSection:Paragraph("SnowmanTimer", {
        Title = "Snowman Event Status",
        Content = "Initializing..."
    })

    task.spawn(function()
        local timerLabel = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("MainUI"):WaitForChild("HUD"):WaitForChild("Bars"):WaitForChild("SnowmanMinigameHUD"):WaitForChild("Stats"):WaitForChild("TimeLeft")
        
        while true do
            pcall(function()
                local currentText = timerLabel.Text
                inMinigameTimer = (currentText ~= "" and currentText) or "N/A"
                ...
                local statusText = status:format(Functions.formatTime(time)) .. "\nIn-Minigame Time: " .. inMinigameTimer
                snowmanTimerParagraph:SetValue(statusText)
            end)
            task.wait(1)
        end
    end)
    ...
    do
        ...
        ChristmasSection:Toggle("AutoBuildSnowman", {
            Title = "Auto Build Snowman",
            Description = "Temporarily changes Auto Hatch and Auto Snow Pile toggles to maximise efficiency.",
            Default = false,
            Callback = function(enabled)
                if autoBuildThread then task.cancel(autoBuildThread); autoBuildThread = nil end
                    ...
                    autoBuildThread = task.spawn(function()
                        ...
                        while task.wait(0.25) do 
                            pcall(function()
                                ...
                                local isInMinigame = inMinigameTimer ~= "N/A" and inMinigameTimer ~= "00:00"
                                
                                -- Check if we're in the reward window (between building end and reward window close)
                                local isRewardWindow = cycle >= 900 and cycle < 3600

                                -- State 1 & 2: Active Phase (Joining or In Minigame)
                                if (isJoiningTime or isInMinigame) and not isRewardWindow then
                                    ...
                                    if not hasPausedOtherToggles then
                                        ...
                                        print("[Snowman] Event active. Saving current settings and applying minigame settings.")

                                        originalToggleStates = {
                                            autoHatch = AutoHatch.Value, 
                                            autoDamage = AutoDamage.Value
                                        }
                                        ...
                                        print("[Snowman] Applied settings - AutoHatch: OFF | AutoDamage: ON")
                                    end
                                    
                                    -- Joining Logic (runs during the 30-second window)
                                    if isJoiningTime then
                                        print("[Snowman] Attempting to join...")
                                        Knit.GetService("SnowmanService"):joinBuildASnowman()
                                        task.wait(9)
                                    end

                                    -- In-Minigame Logic (runs only when inside the minigame)
                                    if isInMinigame then
                                        -- No additional actions needed while in minigame
                                    end

                                -- State 3: Restoration Phase (When reward window starts)
                                elseif hasPausedOtherToggles and isRewardWindow then
                                    print("[Snowman] Reward window active. Restoring original toggle states.")
                                    
                                    -- Restore settings only if we have a saved state
                                    if originalToggleStates then
                                        task.defer(function() 
                                            AutoHatch:SetValue(originalToggleStates.autoHatch)
                                            AutoDamage:SetValue(originalToggleStates.autoDamage)
                                        end)
                                        print("[Snowman] Restored - AutoHatch: " .. tostring(originalToggleStates.autoHatch) .. " | AutoDamage: " .. tostring(originalToggleStates.autoDamage))
                                    end
                                    
                                    -- Reset state for the next cycle
                                    hasPausedOtherToggles = false
                                    print("[Snowman] Restoration complete. Awaiting next event.")
                                end
                            end)
                        end
                    end)
                else 
                    -- This runs when the master toggle is turned OFF
                    if originalToggleStates then
                        print("[Snowman] Master toggle disabled. Restoring all original settings.")
                        task.defer(function()
                            AutoHatch:SetValue(originalToggleStates.autoHatch)
                            AutoDamage:SetValue(originalToggleStates.autoDamage)
                        end)
    ...
    local claimChestThread = nil

    ChristmasSection:Toggle("AutoClaimChest", {
        Title = "Auto Claim Snowman Chest",
        Default = false,
        Callback = function(enabled)
            if claimChestThread then
                task.cancel(claimChestThread)
                claimChestThread = nil
            end

            if enabled then
                claimChestThread = task.spawn(function()
                    while true do
                        pcall(function()
                            Knit.GetService("SnowmanService"):claimBuildASnowmanChest()
                        end)
                        task.wait(10)
                    end
                end)
    ...
    local AutoUseSection = Tabs.Christmas:AddSection("Auto Use Items")
        ...
        -- Build list of usable items (Potions + Boxes)
        ...
        pcall(function()
            ...
            for itemId, itemInfo in pairs(ChristmasItems) do
                -- Check for Potions (multipler + seconds) OR Boxes
                local isPotion = itemInfo.multiplier and itemInfo.seconds
                local isBox = itemInfo.itemType == "box"
                
                -- Filter out tools and unusable items
                if (isPotion or isBox) and not itemInfo.unusable and not itemInfo.isTool then
        ...
        AutoUseSection:Toggle("AutoUseChristmasItems", {
            Title = "Auto Use Selected Items",
            Description = "Uses potions when not active, and opens all selected boxes immediately.",
            Default = false,
            Callback = function(enabled)
                    ...
                    autoUseThread = task.spawn(function()
                        while autoUseEnabled do
                            pcall(function()
                                ...
                                local inventory = playerData.inventory and playerData.inventory.christmasEvent
                                ...
                                for displayName, isSelected in pairs(selectedItems) do
                                    if isSelected then
                                        ...
                                        if itemInfo then
                                            ...
                                            -- If we have the item
                                            if playerItemData and playerItemData.am > 0 then
                                                
                                                -- CASE 1: BOX (Use ALL)
                                                if itemInfo.itemType == "box" then
                                                    Knit.GetService("InventoryService"):useItem(inventoryId, {use = playerItemData.am})
                                                    task.wait(0.5) -- Small delay to prevent spamming too hard
                                                
                                                -- CASE 2: POTION (Check Boosts)
                                                elseif itemInfo.multiplier then
                                                    -- Check boost names
...
local TeleportService = game:GetService("TeleportService")
        ...
        task.spawn(function()
            ...
            Tabs = {
                ...
                Event = Window:AddTab({ Title = "Christmas Event", Icon = "snowflake" }),
